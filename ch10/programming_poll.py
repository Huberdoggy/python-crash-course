...
with open(filename, "a+") as file_object:
    if stat(filename).st_size > 0:
        ...
        print(
            "Files is empty. Writing heading to file prior to commencing the poll..."
        )
...
while True:
    print(
        "You will be asked a series of questions for this poll"
        " (enter 'q' to quit) => "
    )
    ...
    if p_name == "q":
        ...
        poll_response = input(
            "And why do you like programming? (enter 'q' to opt out) => "
        )
        if poll_response == "q":
            break
        else:
