...
class Ship:
    ...
    def update(self):
       ...
       # Update the ship's x value, not the rect (to keep the float portion of our 1.5x speed)
        if self.moving_right and self.rect.right < self.screen_rect.right: # the ship hasn't reached right edge of scr
            self.x += self.settings.ship_speed
        if self.moving_left and self.rect.left > 0: # the ship hasn't reached left edge of screen
            self.x -= self.settings.ship_speed
