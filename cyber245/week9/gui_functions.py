...
def ask_city_or_zip():
    ...
    layout = [
        [sg.Button("Seach by City/State")],
        [sg.Button("Search by Zip Code")],
    ]
    return layout
...
def draw_zip_window():
    ...
    sg.theme("Dark Tan Blue")
    layout = [
        [sg.Image(im, size=(300, 300), pad=(100, 0))],
...
def weather_data_window(city_name, j_dict, im):
    layout = [
        [sg.Image(im, size=(200, 200), pad=(100, 0))],
...
def show_error_window():
    layout = [
        [sg.Text("An error has occured!")],
        [sg.Text("Please check your input and try again")],
        [sg.Button("Ok")],
    ]
    # return layout
...
def check_reg_city_state(pattern_dict, input_var1, input_var2):
    if not (
        re.fullmatch(pattern_dict.get("loc_pattern", "empty"), input_var1)
        and re.fullmatch(
            pattern_dict.get("state_pattern", "empty"), input_var2
        )
    ):
        return False
    else:
...
def check_reg_zip(pattern_dict, input_var1):
    if not (
        re.fullmatch(pattern_dict.get("zip_pattern", "empty"), input_var1)
    ):
        return False
    else:
        return True
