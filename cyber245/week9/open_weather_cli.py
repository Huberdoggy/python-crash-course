...
from cli_functions import (
    compile_patterns,
    build_request_url as build,
    print_the_weather,
    build_request_url_zip as build_z,
    make_request,
)
...
def choice_one():
    while True:
        ...
        if ask == "c":
            ...
            first_reg = raw_reg_dict.get("loc_pattern", "empty")
            second_reg = raw_reg_dict.get("state_pattern", "empty")
            if re.fullmatch(first_reg, city.strip()) and re.fullmatch(
                second_reg, state.strip()
            ):
                check_okay = input(
                    f"You entered '{city.strip()}' and state code '{state.strip()}'. "
                ...
                if check_okay == "y" or check_okay == "Y":
                    ...
                    go_again = input(
                        "Would you like to make a new query? (y/n) => "
                    )
                ...
                print_red(
                    "Invalid input. Ensure only letters are entered for city and state code."
                )
        elif ask == "z":
            zip = input("Enter the five digit zip-code to query => ")
            strip_zip = (
                zip.strip()
            )  # To prevent potential error when sending request. Just like above
            if re.fullmatch(
                raw_reg_dict.get("zip_pattern"), strip_zip
            ):  # Regex handles the rest
                check_okay = input(
                    f"You entered '{strip_zip}'. Is this correct (y/n)? => "
                )
                if check_okay == "y" or check_okay == "Y":
                ...
                print_red(
                    "Invalid zip code. Ensure you enter five digits with no space between."
                )
