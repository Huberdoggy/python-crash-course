...
reg_patterns = {
    'ticker_input': '^[a-zA-Z]{2,5}$'
}

ticker_list = [
    'aapl', 'msft', 'googl', 'amzn',
    'fb', 'tsla', 'nvda', 'jpm', 'wmt', 'unh'
...
while ticker_quote:
    choose_ticker = input(f"\nPlease enter the desired ticker to retrieve current price {or_quit} => ")
    if choose_ticker == 'q':
        break
    elif not re.fullmatch(reg_patterns.get('ticker_input', 'empty'), choose_ticker):
        print_red(f"Your input {choose_ticker.strip()} is not in the correct format."
                  f"\nEnsure 2-5 letters only\nWith no space in between.")
    else:
        choose_ticker = choose_ticker.strip().lower() # ensure surrounding white space is removed and input matches
        # dict case
        if choose_ticker in ticker_dict:
            color_ticker = colored(choose_ticker.upper(), 'green') # format substring in color for easy sighting
            ...
            print(colored(f"\n\t\t*** DATA FROM THE PAST WEEK FOR {choose_ticker.upper()} ***", 'green'))
            print(f"\n{stock_table}")
