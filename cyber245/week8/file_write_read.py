...
reg_patterns = {
    ...
    'phone_num': '^\d{3}\d{3}\d{4}$', # for user convenience, I wont require hyphens or literal parenthesees
}

user_info = {}

c_p(reg_patterns)
prog_file_path = exp_vars('$HOME/Documents') # Pass string to func to expand $HOME on Linux
s_c()
...
while not name_valid:
    ...
    if re.fullmatch(reg_patterns.get('name', 'empty'), get_name):
    ...
    while not address_valid:
        ...
        if re.fullmatch(reg_patterns.get('address', 'empty'), get_addy):
            user_info['Address'] = get_addy
            address_valid = True
        else:
        ...
        while not phone_valid:
            ...
            if re.fullmatch(reg_patterns.get('phone_num', 'empty'), get_phone):
