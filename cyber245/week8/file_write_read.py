...
from file_funcs import (
    add_to_path as atp,
    expand_env_vars as exp_vars,
    does_fpath_exist as does_exist,
    store_data,
)

mod_paths = [
    "/home/huberdoggy/python-projects/python-crash-course/cyber245/week9",
    "/home/huberdoggy/python-projects/python-crash-course/cyber245/week6",
]
atp(
    mod_paths
)  # Quickly insert addl paths into $PYTHONPATH for my other module dirs
...
s_c()
full_file_path = does_exist(
    prog_file_path, reg_patterns
)  # will return True if my base-path variable exists and
...
while not name_valid:
    ...
    if re.fullmatch(reg_patterns.get("name", "empty"), get_name.strip()):
        ...
        print(
            colored(
                f"{get_name.strip()} is not the correct format for 'name'.",
                "red",
            )
        )
        continue
    while not address_valid:
        ...
        if re.fullmatch(
            reg_patterns.get("address", "empty"), get_addy.strip()
        ):
            ...
            print(
                colored(
                    f"{get_addy.strip()} is not the correct format for 'address'.",
                    "red",
                )
            )
            continue
        while not phone_valid:
            ...
            if re.fullmatch(
                reg_patterns.get("phone_num", "empty"), get_phone.strip()
            ):
                ...
                print(
                    colored(
                        f"{get_phone.strip()} is not the correct format for 'phone'.",
                        "red",
                    )
                )
