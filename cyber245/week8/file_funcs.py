...
def does_fpath_exist(path_str, pattern_dict):
    if os.path.isdir(path_str):
        while True: # Technically, this will only go until a file-name matches my naming convention, since it will
            # return 'filename' at that point
            filename = input(f"Will store your file under {colored(path_str, 'green')}.\n"
...
def store_data(file_name, user_info): # take input vars and the full file path from above function.
