...
def screen_clear():
    ...
    if name == "nt":  # In the rarer circumstance I'm running on Windows
        _ = system("cls")
        # Store in underscore as variable to align with the fact a Py shell always stores its last output in an underscore
    else:  # For 'Nix
        _ = system("clear")
...
def format_main_menu(render_str, m_dict):
    ...
    print_green(
        f"\tPlease enter a number corresponding to one of the following:\n"
    )
    print("\t" * 3 + ("*" * 30))
    for key in m_dict:
...
class Vehicle:
    def __init__(
        self, engineSize=None, numDoors=None, cabStyle=None, bedLength=None
    ):
    ...
    def build_opts_menu(
        self,
    ):  # First, I'm going to build a dict of opts, similar to main-menu
        my_dict = {}
        count = 1
    ...
    def getOptions(
        self, render_str, m_dict
    ):  # Now I'm passing the dictionary to my 'cool formatting' func
        screen_clear()
        f = Figlet(font="standard", justify="center")
        print(f"{colored(f.renderText(render_str), 'blue')}\n")
        print_green(
            f"\tPlease enter numbers corresponding to the following options\n"
        ...
        try:
            ...
            while my_flag:
                num_choice = input(
                    "Enter a number to add that option to your vehicle (or 'q' to return) => "
                )
                if re.fullmatch(pattern2, num_choice.strip()):
                    if num_choice == "q" and len(chosen_opt_lst) < 1:
                        ...
                        print_red(
                            f"Input {num_choice.strip()} is not an option."
                        )
...
class Truck(Vehicle):
    def __init__(self):
        ...
        self.options = Truck.getOptions(
            self, "Vehicle Options", truck_opts_dict
        )
